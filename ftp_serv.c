/** 익명의 ftp 기능을 지원하는 ftp 서버 프로그램
 * 최종 작성 일시 : 2020-11-02
 * 
 * [프로그램 기능]
 * ftp 서버의 파일 목록을 보여주는 기능 제공 (ls)
 * ftp 서버에 존재하는 파일을 클라이언트가 다운받을 수 있는 기능 제공 (get)
 * 클라이언트가 ftp 서버에 파일을 업로드할 수 있는 기능 제공 (put)
 * 
 * [프로그램 설명]
 * 해당 ftp 서버는 multi thread를 통해 동시에 여러 클라이언트를 처리하는 multi server
 * 클라이언트가 파일 업로드/다운로드 시, mutex lock을 이용, 임계구역 처리
 * 클라이언트의 이용 내역을 간단하게 기록하는 날짜별 log 파일 생성
 * 접속자가 너무 많을 시, 접속자가 너무 많음을 클라이언트에게 알리고 연결 해제(현재 최대 10명)
 * 클라이언트 파일 업로드/다운로드 시, file size를 비교하여 제대로 전송되었는지 확인
 * 클라이언트가 파일 업로드 시, 이미 존재하는 파일인 경우 업로드를 막음
 *
 */
