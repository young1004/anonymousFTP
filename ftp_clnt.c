/** 익명의 ftp 기능을 지원하는 ftp 클라이언트 프로그램
 * 최종 작성 일시 : 2020-11-02
 * 
 * [프로그램 기능]
 * ftp 서버와 클라이언트의 파일 목록을 보여주는 기능 제공 (ls)
 * ftp 서버에 존재하는 파일을 다운받을 수 있는 기능 제공 (get)
 * ftp 서버로 클라이언트의 파일을 업로드할 수 있는 기능 제공 (put)
 * 다운로드/업로드 시, 진행률 보기를 on/off 할 수 있는 기능 제공 (hash)
 * 문자 'q'를 입력하여 프로그램 종료
 * 
 * [프로그램 설명]
 * 해당 ftp 클라이언트는 ftp 서버에 대응하여 read/write를 통해 서버와 통신
 * hash 기능을 통해 ftp 서버로부터 파일 다운로드/업로드 시 진행률 확인 가능
 * 서버에 명령어를 보내기 전, 클라이언트의 각종 잘못된 입력을 제어하는 예외처리 기능 존재
 * ftp 서버로부터 받은 데이터를 통해 각종 예외를 출력
 * 이미 다운로드 받았던 파일을 다시 다운로드하는 경우, 파일명을 변경하여 다시 받아옴
 * 
 */
